...
def run():
    ...
    with open('out.txt', 'w') as out, open('err.txt', 'w') as err:
        ...
        try:
            ...
            try:
                subprocess.run('../../../build/gentool ../basic.json', **kwargs)
            except subprocess.CalledProcessError:
                raise ValueError("Binding generation failed")
            try:
                subprocess.run(f'dmd ../d/basic.d generated.d {config}/basic{libExt} {" ".join(extra_args)} -unittest -main -g', check=True)
            except subprocess.CalledProcessError:
